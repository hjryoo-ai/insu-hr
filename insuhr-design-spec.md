...
> **버전**: 2.0 / 작성일: 2026-07-17 / 최종 개정: 2026-07-18 ([개정 이력](#개정-이력))
...
- **진행 상태 (v2.0):** 완료 기준 3잡(`eligibilityRefreshJob`·`futureAppointApplyJob`·`hrSnapshotFileJob`) + V14(Batch 메타테이블) 그린 — 위 완료 기준 4항 전부 통과. **`phase-7` 태그는 나머지 6잡 완료까지 보류**(태그 의미를 §13.2 기준과 §8 로스터 사이에서 가르지 않기 위함). 남은 6잡(`guaranteeExpiry`·`continuingEduNotice`·`licenseValidity`·`annualLeaveGrant`·`outboxDlqSweep`·`dataQuality`)은 **알림/리포트 출력 테이블 V15(`TB_NOTICE_QUEUE`/`TB_DQ_FINDING`, 8장 설계)** 신설이 선행. 후속 세션 완료 기준: V15 + 6잡 + 각 잡 dedup 키 재실행 멱등 테스트 + 알림 이벤트 조건부 발행.
