...
> **버전**: 2.3 / 작성일: 2026-07-17 / 최종 개정: 2026-07-18 ([개정 이력](#개정-이력))
...
### 13.4 v1.1 백로그 (릴리스 후 우선순위)

v1.0에서 의도적으로 뺀 것들을 우선순위 순으로 남긴다 — [README](./README.md)의 "v1.1 백로그"와 같은 순서다. 무엇을 왜 안 만들었는지가 기록으로 남아야 다음 사람이 되풀이하지 않는다.

1. **변경감사 AOP + `TB_AUDIT_LOG`** (§6.6·§10.4) — 여덟 Phase 어느 완료 기준에도 안 걸린 백로그(이력이 이벤트형·유효기간형으로 이미 촘촘해 세 번째 겹). 구현 시 `TB_PERSON` 민감필드를 감사 JSON에서 **소스 제외**해야 한다(파기 정합성 전제, §10.4).
2. **`PUT /auth/password` + 비밀번호 이력·90일 변경 집행** (§10.1) — 정책값(`PWD_EXPIRE_DAYS`·`PWD_REUSE_BLOCK_CNT`)은 시드됐으나 엔드포인트·집행은 미구현.
3. **Pull API 소비자 관점 계약 테스트** (§9.4) — 현재는 생산자 측 워터마크·커서 테스트만 있다. 수신측 재개·유실복구 계약을 소비자 관점 계약 테스트로.
4. **릴레이 스케일아웃 — aggId 해시 파티셔닝** (§9.2) — 현재는 단일 릴레이 인스턴스 가정(§9.2의 문서화된 트레이드오프). 다중 인스턴스는 aggId 파티셔닝이 전제.

릴리스 규율:

- **태그 대상 = 배포 산출물(5개 모듈의 코드·설정)뿐이다.** 문서·데모 스크립트·CI 툴링(`.github/`·`.gitleaks.toml`·`.env.example`·`demo/`) 변경은 릴리스된 애플리케이션 동작을 바꾸지 않으므로 태그 없이 쌓는다 — "설정"이 앱 런타임 설정인지 저장소 툴링인지가 판단 지점이며, 후자는 제외다.
- 배포 산출물의 코드/설정이 바뀌면 `v1.0.x`, 그 외(문서·툴링)는 태그 없음.
- **`v1.0` 릴리스 태그는 옮기지 않는다**(마이그레이션 불변성 정책과 같은 규율).
...
| 버전 | 일자 | 내용 |
|---|---|---|
| 2.3 | 2026-07-18 | **v1.0 공개 준비 (문서 전용, 태그 없음)** — ① **v1.1 백로그 서열화**(§13.4 신설): 변경감사 AOP → 비밀번호 변경 집행 → Pull 소비자 계약 테스트 → 릴레이 스케일아웃 순, [README](./README.md)와 동일 순서로 상호 링크 ② **릴리스 규율 명문화**(§13.4): 릴리스 후 코드/설정 변경은 `v1.0.x`, 문서만이면 태그 없음, **`v1.0` 태그는 불변**(마이그레이션 불변성과 같은 규율) ③ README를 포트폴리오 프런트페이지로 재작성(수치 전수 검증: 168 테스트·V17·잡 10·전이 7/18·common 의존성 0)하고 설계서와 상호 링크. 코드·스키마 무변경 |
